...
public class Main {
	...
	public static void main( String args[] ) throws SecurityException, UnsupportedEncodingException {
		...
		theProperties.withName( "six" ).withTitle( ">>> " + "six".toUpperCase() + " >>>" ).withCopyrightNote( "Copyright (c) by six" ).withLicenseNote( "Licensed under GNU General Public License, v3.0 and Apache License, v2.0" );
		...
		try {
			...
			try {
				...
				theProperties.withFilePath( "six.ini" );
				// <--| Put an "*.ini" file as sepcified in the documentation 
			}
			catch ( IOException ignore ) {
				// Continue in case there is none sich "*.ini" file |-->
				if ( theProperties.getBoolean( "verbose" ) ) {
					LOGGER.warn( "No config file with name <six.ini> found, skipping." );
				}
				// <--| Continue in case there is none sich "*.ini" file
			}
			if ( theProperties.getBoolean( "verbose" ) ) {
				LOGGER.info( "Starting application <six>..." );
